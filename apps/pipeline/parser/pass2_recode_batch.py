...
Lives in parser/ (not scripts/) so both the live pipeline
(orchestrator/pipeline.py's lite-gated pass-2 stage) and the ops backfill
script (scripts/recode_cycle_pass2.py, a thin re-export of this module)
call the exact same orchestration — never two copies of the batching/
retry logic.
