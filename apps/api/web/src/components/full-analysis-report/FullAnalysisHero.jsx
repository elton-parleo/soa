/**
 * Full-analysis hero — mirrors ScoreHero.jsx (lite/report/ScoreHero.jsx)
 * markup/tokens exactly (same PaceLane math, same panel/lane/tile
 * layout), NOT forked from it: ScoreHero itself isn't reusable as-is
 * because it hardcodes a single ChatGPT platform tag and LITE_QUERY_
 * COUNT, both wrong for a multi-platform, real-total_queries full
 * cycle. Per the task's own fallback rule ("mirror the shipped markup
 * exactly... leave lite alone"), this is a sibling, not a modification.
 */
...
export function FullAnalysisHero({ report, exposure, shareOfMentionsRank, headline, platforms }) {
    ...
    <div id="score" style={{ marginBottom: 18, scrollMarginTop: 26 }}>
      <DarkPanel pad={0} radius={18} atmos style={{ overflow: 'hidden', boxShadow: 'var(--shadow-elevated)' }}>
        <div style={{ padding: '28px 32px 26px', borderBottom: '1px solid var(--dark-border)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
            ...
            <span style={{ marginLeft: 'auto', display: 'flex', gap: 7, flexWrap: 'wrap' }}>
              {platforms.map((p) => (
                <MonoTag key={p} logo={PLATFORM_LOGO[p]} tone="dark">{PLATFORM_LOGO[p] || p}</MonoTag>
              ))}
              <MonoTag tone="dark">{report.total_queries} QUERIES</MonoTag>
            </span>
          </div>
          ...
          <div style={{ marginTop: 26, padding: '22px 24px 20px', background: 'rgba(242,240,239,.055)', border: '1px solid var(--dark-border)', borderRadius: 15 }}>
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: 22, flexWrap: 'wrap' }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 7 }}>
                <span className="num" style={{ fontSize: 66, fontWeight: 750, letterSpacing: '-0.044em', lineHeight: 0.86, color: 'var(--dark-text)' }}>
            ...
            <div style={{ marginTop: 24 }}>
              <div style={{ background: 'var(--canvas)', borderRadius: 14, padding: '22px 22px 18px', boxShadow: 'inset 0 1px 0 rgba(255,255,255,.7),0 10px 26px -12px rgba(10,10,18,.6)' }}>
                ...
                <div className="fa-hero-lanes" style={{ display: 'flex', gap: 27, alignItems: 'flex-start' }}>
                  <PaceLane label={PILLAR_NAMES[PILLAR_VISIBILITY]} earned={vis.earned} fullMax={pillarNominalWeight(PILLAR_VISIBILITY)} />
                  <PaceLane label={PILLAR_NAMES[PILLAR_ACCESSIBILITY]} earned={acc.earned} fullMax={pillarNominalWeight(PILLAR_ACCESSIBILITY)} />
                  <PaceLane label={PILLAR_NAMES[PILLAR_TRUE_VALUE]} earned={tv.earned} fullMax={pillarNominalWeight(PILLAR_TRUE_VALUE)} isTrueValue />
                </div>
              </div>
            </div>
          </div>

          <div className="fa-hero-tiles" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginTop: 16 }}>
        ...
        <div style={{ padding: '26px 32px 28px' }}>
          <div className="fa-hero-pillars" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14 }}>
            {pillarCards.map((p) => {
